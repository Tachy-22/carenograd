            ...
            <div key={message.id}>
              <Message from={message.role}>
                <MessageContent>
                  ...
                  {/* Show tools/tasks only during streaming and in development */}
                  {process.env.NEXT_PUBLIC_NODE_ENV !== 'production' && status === "streaming" && message.parts?.some(part => part && part.type?.startsWith('tool-')) && (
                    <Task>
                      <TaskTrigger title="Tasks" />
                      <TaskContent className="bg-gray-50/50 border border-gray-300 rounded-lg mt-2 p-2">
                        {message.parts
                          ?.filter(part => part && part.type && part.type.startsWith('tool-'))
                          ?.map((part, i) => {
                            if (!part) return null
                  ...
                  {message.parts?.map((part, i) => (
                    <Fragment key={`${message.id}-part-${i}`}>
                      {part.text && (
                  ...
                  {/* Show loading dots for streaming assistant messages */}
                  {status === "streaming" && message.role === 'assistant' && (!message.parts || message.parts.length === 0 || !message.parts.some(part => part.type === 'text' && part.text)) && (
